...
@Slf4j
public class WeChatWorkProfileDefinition extends OAuth20ProfileDefinition<OAuth20Profile, OAuth20Configuration> {

    private static final String DEFAULT_PROFILE_ID_KEY = "UserId";
    ...
    public WeChatWorkProfileDefinition() {
        this.setProfileId(DEFAULT_PROFILE_ID_KEY);
    }

    @Override
    public String getProfileUrl(OAuth2AccessToken accessToken, OAuth20Configuration configuration) {
        return ((WeChatWorkApi) configuration.getApi()).getProfileUrl();
    }
    ...
    public OAuth20Profile extractUserProfile(String body) {
        ...
        try {
            ...
            if (tokenResponseSuccess(userJson)) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug(String.format("WeChatWork user related to: %s", JSON.toJSONString(userEntity)));
                }
                userJson.putAll(userEntity);

                if (userJson.get(getProfileId()) == null) {
                    throw new OAuthCredentialsException("profile not found");
                }
    ...
    private boolean tokenResponseSuccess(JSONObject body) {
